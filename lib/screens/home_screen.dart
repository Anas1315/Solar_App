...
class _HomeScreenState extends State<HomeScreen>
  ...
  Widget build(BuildContext context) {
    ...
    final values = _EnergyValues.fromData(data);
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          const Icon(Icons.notifications, color: AppTheme.error, size: 19),
          const SizedBox(width: 5),
          const Text(
            'Alert',
            style: TextStyle(
...
class _EnergyScene extends StatelessWidget {
  final double progress;
  final _EnergyValues values;
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final points = _ScenePoints(Size(width, height));
        ...
        return Stack(
          ...
          children: [
            ...
            _PositionedLabel(
              left: points.houseRect.left + points.houseRect.width * 0.22,
              top: points.houseRect.top - (compact ? 54 : 68),
              child: _MetricLabel(
                valueText: _formatNumber(values.consumptionKw),
                unit: 'kW',
                label: 'Consumption',
            ...
            _PositionedLabel(
              right: math.max(12, width - points.gridPoleBase.dx - 34),
              top: points.gridPoleBase.dy - 148,
              child: _MetricLabel(
                valueText: _formatNumber(values.gridVoltage),
                unit: 'V',
                label: 'Grid Voltage',
...
class _LoadLineLabel extends StatelessWidget {
  final String text;
  final Color color;
  final bool brightText;

  const _LoadLineLabel({
    required this.text,
    required this.color,
    required this.brightText,
  });
...
class _EnergyScenePainter extends CustomPainter {
  final double progress;
  final _EnergyValues values;
  ...
  void paint(Canvas canvas, Size size) {
    final points = _ScenePoints(size);
  ...
  void _drawWeather(Canvas canvas, Size size, _ScenePoints points) {
    if (!values.isDayTime) {
  ...
  void _drawSunRaysToPanel(Canvas canvas, Offset sun, Rect panel) {
    ...
    for (int i = 0; i < rayCount; i++) {
      ...
      final target = Offset(
        ...
        panel.top + panel.height * (0.2 + 0.08 * math.sin(progress * 6 + i)),
      );
      final dashProgress = (progress + i * 0.16) % 1;
  ...
  void _drawClouds(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 4; i++) {
      final drift = ((progress + i * 0.23) % 1) * (size.width + 180) - 90;
  ...
  void _drawLandscape(Canvas canvas, Size size, _ScenePoints points) {
  ...
  void _drawEnergyLines(Canvas canvas, Size size, _ScenePoints points) {
    final sourceBus = points.sourceBusY;
    final loadBus = points.loadBusY;
    final paths = [
      _EnergyFlow(
        path: _polylinePath([
          points.solarToInverter,
          Offset(points.solarToInverter.dx, sourceBus - 20),
          Offset(points.inverterSolarPort.dx, sourceBus - 20),
      ...
      _EnergyFlow(
        path: _polylinePath([
          points.batteryToInverter,
          Offset(points.batteryToInverter.dx, sourceBus + 4),
          Offset(points.inverterBatteryPort.dx, sourceBus + 4),
      ...
      _EnergyFlow(
        path: _polylinePath([
          ...
          Offset(points.gridToInverter.dx, sourceBus + 28),
          Offset(points.inverterGridPort.dx, sourceBus + 28),
      ...
      _EnergyFlow(
        path: _polylinePath([
          ...
          Offset(points.inverterSmartLoadPort.dx, loadBus),
          Offset(points.smartLoadInput.dx, loadBus),
      ...
      _EnergyFlow(
        path: _polylinePath([
          ...
          Offset(points.inverterHeavyLoadPort.dx, loadBus + 24),
          Offset(points.heavyLoadInput.dx, loadBus + 24),
          points.heavyLoadInput,
        ]),
  ...
  void _drawHome(Canvas canvas, Rect rect) {
    ...
    _drawWindow(canvas,
        Rect.fromLTWH(sideRect.left + 24, sideRect.top + 30, 16, 34), window);
  }
  ...
  void _drawBattery(Canvas canvas, Rect rect) {
    ...
    for (int i = 0; i < 4; i++) {
      canvas.drawRRect(
        RRect.fromRectAndRadius(
          Rect.fromLTWH(
              rect.left + 8 + i * 10, rect.top + 12, 7, rect.height - 22),
  ...
  @override
  bool shouldRepaint(_EnergyScenePainter oldDelegate) {
...
class _ScenePoints {
  ...
  Offset get heavyLoadInput => Offset(
      houseRect.center.dx + houseRect.width * 0.18, houseRect.bottom - 4);
}
...
class _EnergyValues {
  ...
  factory _EnergyValues.fromData(EnergyData? data) {
    final powerKw =
        data == null ? 456.0 : math.max<double>(0, data.power / 1000);
    final hasPower = powerKw > 0;
    final gridActive =
        data == null ? true : data.wapdaAvailable && data.wapdaRelayState;
    final heavyLoad = data == null ? true : data.heavyLoadState;
    final ldr = data?.ldrValue ?? 2200;
    final isNight = ldr <= 0;
    ...
    return _EnergyValues(
      powerKw: powerKw,
      productionKw: powerKw,
      consumptionKw: data == null ? 456.0 : powerKw,
      batteryKw: data == null
          ? 456.0
          : hasPower
      ...
      gridKw: data == null
          ...
          : gridActive
              ...
              : 0,
      ldrValue: ldr,
      gridVoltage: data?.voltage ?? 219.0,
